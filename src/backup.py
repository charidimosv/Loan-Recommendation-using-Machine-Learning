...
def rmsle_cv(model):
    ...
    rmse = np.sqrt(-cross_val_score(model, train.values, y_train, n_jobs=-1, scoring="neg_mean_squared_error", cv=kf))
    return rmse
...
if __name__ == "__main__":
    train = df_train = pd.read_csv(TRAIN_PATH)
    test = df_test = pd.read_csv(TEST_PATH)
    ...
    print("The train data size before: {} ".format(train.shape))
    ...
    print("The test data size before: {} ".format(test.shape))
